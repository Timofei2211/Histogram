...
public class DrawingComponent4 extends JPanel{
    ...
    public DrawingComponent4(HistogramModel histogramModel) {
    ...
    protected void paintComponent(Graphics gh) {
        ...
        for (int i = 0; i <19 ; i++) {
            ...
            drp.drawString(String.valueOf(df2.format((min) + (i * h))), 110+50*i, 420);
        ...
        for (int i=0; i<10; i++) {
            ...
            for (int j=0;j<20;j++) {
                try {
                    Field field = Class.forName("java.awt.Color").getField(GrGis4.col[j].toLowerCase());
                    color1 = (Color)field.get(null);
                } catch (Exception e) {}
                drp.setColor(color1);
                //переводим полученные данные в реальные координаты
                int realY = (int) (400- GrGis4.y[j][i]/2)+3;
                drp.fillRect(70+50*j+100*i, realY, 50,(int) (GrGis4.y[j][i]/2));
            }
        }
    }
}